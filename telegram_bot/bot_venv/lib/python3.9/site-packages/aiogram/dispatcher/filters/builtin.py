...
class Command(Filter):
    ...
    def __init__(
        self,
        commands: Union[Iterable[Union[str, BotCommand]], str, BotCommand],
        ...
        if isinstance(commands, (str, BotCommand)):
            ...
            raise ValueError(
                ...
                "It only supports str, BotCommand object or their Iterable".format(
                    type(commands)
                )
            )
        commands = [
            cmd.command if isinstance(cmd, BotCommand) else cmd for cmd in commands
        ]
    ...
    async def check_command(
        ...
        return {
            "command": cls.CommandObj(
                command=command, prefix=prefix, mention=mention, args=args
            )
        }
...
class StateFilter(BoundFilter):
    ...
    def get_target(self, obj):
        if isinstance(obj, CallbackQuery):
            return getattr(
                getattr(getattr(obj, "message", None), "chat", None), "id", None
...
class IDFilter(Filter):
    ...
    async def check(
        self, obj: Union[Message, CallbackQuery, InlineQuery, ChatMemberUpdated]
    ):
        if isinstance(obj, Message):
            user_id = None
...
class ChatTypeFilter(BoundFilter):
    ...
    async def check(
        self, obj: Union[Message, CallbackQuery, ChatMemberUpdated, InlineQuery]
    ):
        if isinstance(obj, Message):
