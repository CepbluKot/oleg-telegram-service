...
class BaseBot:
    ...
    @contextlib.contextmanager
    def with_token(
        self, bot_token: base.String, validate_token: Optional[base.Boolean] = True
    ):
        if validate_token:
    ...
    async def download_file(
        ...
        if (
            make_dirs
            and not isinstance(destination, io.IOBase)
            and os.path.dirname(destination)
        ):
    ...
    async def send_file(
        self, file_type, method, file, payload
    ) -> Union[Dict, base.Boolean]:
        """
        Send file
    ...
    def parse_mode(self, value):
        if value is None:
            ...
            if value == "markdown":
                warnings.warn(
                    "Parse mode `Markdown` is legacy since Telegram Bot API 4.5, "
    ...
    def disable_web_page_preview(self, value):
        if value is None:
            ...
            if not isinstance(value, bool):
                raise TypeError(
                    f"Disable web page preview must be bool, not {type(value)}"
                )
