...
async def get_bookings(message: types.Message):
    bookings = await booking_repository_abstraction.get_users_bookings(3)

    user_boking_data = booking_viewer_repository.read(message.chat.id)

    if not user_boking_data:
        user_boking_data = BookingMenu(page_id=0, current_message_id=-1)
        booking_viewer_repository.create(tg_id=message.chat.id, data=user_boking_data)
    
    else:
        await bot.delete_message(message.chat.id, user_boking_data.current_message_id)

    booking_message_text, buttons = messages.booking_view_menu_message(bookings, user_boking_data.page_id)
    ...
    booking_viewer_repository.update(tg_id=message.chat.id, data=BookingMenu(page_id=0, current_message_id=answer_msg.message_id))
